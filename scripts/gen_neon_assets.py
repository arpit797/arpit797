#!/usr/bin/env python3
"""
Build the animated SVG art for the profile README of Arpit Yadav (arpit797).

Pure SMIL SVG animations that run infinitely and look stunning in both light & dark mode.
"""
...
def hero(theme: Theme) -> str:
    ...
    parts.append(
        ...
        'font-size="86" font-weight="800" letter-spacing="3" fill="#fff">'
        "ARPIT YADAV</text>"
        "</mask>"
        "</defs>"
    )
    ...
    parts.append(
        f'<text x="600" y="150" text-anchor="middle" font-family="{FONT}" font-size="86" '
        'font-weight="800" letter-spacing="3" fill="url(#ink)">ARPIT YADAV</text>'
    )
    ...
    # Subtitle, letter-spaced and sleek.
    parts.append(
        f'<text x="600" y="188" text-anchor="middle" font-family="{MONO}" font-size="15" '
        f'letter-spacing="7" fill="{theme.muted}">FULL STACK DEVELOPER  ·  MERN  ·  PROBLEM SOLVER</text>'
    )

    # Rule under everything, with a bright travelling segment.
    parts.append(
        f'<rect x="300" y="208" width="600" height="2" rx="1" fill="url(#rule)" opacity="0.5"/>'
        f'<rect x="300" y="208" width="90" height="2" rx="1" fill="{theme.neon[0]}">'
...
# ─────────────────────────────────────────────────────────────────────────────
# 3. Pipeline: packets travelling through the full-stack stages
# ─────────────────────────────────────────────────────────────────────────────
def pipeline(theme: Theme) -> str:
    W, H = 1200, 170
    stages = ["DESIGN", "BUILD", "CONNECT", "SCALE", "DEPLOY"]
    n = len(stages)
    pad, y = 130, 74
    step = (W - pad * 2) / (n - 1)
